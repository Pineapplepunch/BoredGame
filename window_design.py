...
class HorizontalMeterBar(tk.Frame):  # (parent,title,[width,height,labelpos,clickable)
    ...
    def __init__(self, master, current, max, type, cwidth=100, cheight=50, labelpos='in', font=None, **args):
        ...
        self.label_id = self.canvas.create_text(self.cwidth / 2, (self.cheight / 2) + 2,
                                                text='{0}/{1}'.format(self.current, self.max), justify='center')
    ...
    def generate_next_level(self):
        if self.current_floor:
            ...
            if (self.current_floor.floor_index + 1) % 10 == 5:
                self.floors_arr['floor' + str(self.current_floor.floor_index + 1)] = objects.generate_shop(
                    self.current_floor.floor_index, self.player_obj)
            elif (self.current_floor.floor_index + 1) % 10 == 0 and self.current_floor.floor_index != 0:
                self.floors_arr['floor' + str(self.current_floor.floor_index + 1)] = objects.generate_bonus(
                    self.current_floor.floor_index, self.player_obj)
            else:
                self.floors_arr['floor' + str(self.current_floor.floor_index + 1)] = objects.generate_next_level(
                    self.current_floor.floor_index, self.player_obj)
            self.charinfo.tower.increment()
        else:
            if self.istutorial:
                self.floors_arr['floor-5'] = objects.generate_next_level(-6, self.player_obj)
                self.current_floor = self.floors_arr['floor-5']
            else:
                self.floors_arr['floor0'] = objects.generate_next_level(-1, self.player_obj)
    ...
    def button_handle(self, event):
        ...
        if len(arr[1]) > 0:
            ...
            if arr[2] == 'floor':  # Fix This
                ...
                s = self.current_floor.events[(self.current_floor.player_pos[0] + directions[event][0],
                                               self.current_floor.player_pos[1] + directions[event][1])].shop
                print(s)
                self.key_bind_unset()
                self.gameboard.create_shop_window(s)
            elif arr[2] == 'combat':
                ev = self.current_floor.events[(self.current_floor.player_pos[0] + directions[event][0],
                                                self.current_floor.player_pos[1] + directions[event][1])]
...
class Character_Creation(tk.Frame):  # (parent)#implement Load character
    ...
    def menu(self):
        ...
        self.check_tutorial = tk.Checkbutton(self, text='Enable Tutorial', variable=self.tutorial_enabled, onvalue=1,
                                             offvalue=0)
...
class Combat_Window(tk.Frame):  # (parent,eventobj,prevDirection)
    ...
    def status_ui(self):
        ...
        self.p_labels = [
            ...
            HorizontalMeterBar(self.player_frame, self.master.player_obj.currhp, self.master.player_obj.maxhp, 'hp',
                               cheight=25, font=self.master.gamefont),
            ...
            HorizontalMeterBar(self.player_frame, self.master.player_obj.currmp, self.master.player_obj.maxmp, 'mp',
                               cheight=25, font=self.master.gamefont),
        ...
        self.e_labels = [
            ...
            HorizontalMeterBar(self.enemy_frame, self.monster.currhp, self.monster.maxhp, 'hp', cheight=25,
    ...
    def menu_ui(self):
        ...
        self.start_button = HoverButton(self.b_frame, text='Start\nBattle', height=5, width=10,
                                        activebackground='lightblue', activeforeground='black',
                                        command=self.advance_turn)
        self.run_button = HoverButton(self.b_frame, text='Run', height=5, width=10, activebackground='red',
                                      activeforeground='black', command=self.run_battle)
        self.items_button = HoverButton(self.b_frame, text="Items", height=5, activebackground='lightblue',
                                        activeforeground='black', width=10, command=self.combat_item_menu)
        self.spells_button = HoverButton(self.b_frame, text="Spells\nNot done", height=5, width=10,
                                         activebackground='lightblue', activeforeground='black',
                                         command=self.combat_spell_menu)
    ...
    def combat_item_menu(self):  # make a turn pass
        ...
        HoverButton(self.i_frame, text='Back', activebackground='lightgrey', activeforeground='black',
                    command=back).pack(side='top', fill='x')
    ...
    def combat_spell_menu(self):  # make a turn pass
        ...
        def get_equipped_spells():
            if self.player.equipped_spells['level:1'] != None:
                ...
                self.level_1.configure(text=self.player.equipped_spells['level:1'].name,
                                       activebackground=get_color('level:1'), activeforeground='black')
            if self.player.equipped_spells['level:2'] != None:
                self.level_2.configure(state='normal')
                self.level_2.configure(text=self.player.equipped_spells['level:2'].name,
                                       activebackground=get_color('level:2'), activeforeground='black')
            if self.player.equipped_spells['level:3'] != None:
                self.level_3.configure(state='normal')
                self.level_3.configure(text=self.player.equipped_spells['level:3'].name,
                                       activebackground=get_color('level:3'), activeforeground='black')
            if self.player.equipped_spells['level:4'] != None:
                self.level_4.configure(state='normal')
                self.level_5.configure(text=self.player.equipped_spells['level:4'].name,
                                       activebackground=get_color('level:4'), activeforeground='black')

        self.b_frame.pack_forget()
        self.sp_frame = tk.Frame(self.lowerframe)
        self.sp_frame.pack(fill=tk.BOTH, expand=1)
        HoverButton(self.sp_frame, text='Back', activebackground='lightgrey', activeforeground='black',
                    command=back).pack(side='top', fill='x')
    ...
    def check_win(self, target):
        if target.is_dead():
            ...
            self.message.config(text=self.combat_log[-1])
            self.b_frame.pack_forget()
            HoverButton(self.lowerframe, text='Combat Complete', activebackground='green', activeforeground='white',
                        command=lambda e=self.player.is_dead(): self.complete(e)).pack(fill=tk.BOTH, expand=1)
    ...
    def fill_inventories(self):
        ...
        for item in self.master.player_obj.inventory:
            if isinstance(item, objects.Potion):
                ...
                if item.slot == 'weapon':
                    self.pi.insert('', 'end', text=item.name,
                                   values=(f'{item.bonus[0]} Att', f"Hit% {int(item.bonus[1] * 100)}", "", item.price))
                else:
                    self.pi.insert('', 'end', text=item.name,
                                   values=(f"{item.bonus} Def", f'{item.slot}', "", item.price))
            elif isinstance(item, objects.Spell_book):
                if item.spellvalue[0] == 'self':
                    self.pi.insert('', 'end', text=item.name + '(Spellbook)', values=(
                    f"{item.spellslot}", f'{item.spellvalue[1]}HP', f"{item.cost}MP", item.price))
                else:
                    self.pi.insert('', 'end', text=item.name + '(Spellbook)', values=(
                    f"{item.spellslot}", f'{item.spellvalue[1]}Dmg', f"{item.cost}MP", item.price))
        ...
        for item in self.shop.shop_inventory:
            if isinstance(item, objects.Potion):
                ...
                if item.slot == 'weapon':
                    self.si.insert('', 'end', text=item.name,
                                   values=(f'{item.bonus[0]} Att', f"Hit% {int(item.bonus[1] * 100)}", "", item.price))
                else:
                    self.si.insert('', 'end', text=item.name,
                                   values=(f"{item.bonus} Def", f'{item.slot}', "", item.price))
                ...
                if item.spellvalue[0] == 'self':
                    self.si.insert('', 'end', text=item.name + '(Spellbook)', values=(
                    f"{item.spellslot}", f'{item.spellvalue[1]}HP', f"{item.cost}MP", item.price))
                else:
                    self.si.insert('', 'end', text=item.name + '(Spellbook)', values=(
                    f"{item.spellslot}", f'{item.spellvalue[1]}Dmg', f"{item.cost}MP", item.price))
...
class Portrait(tk.Frame):  # (parent)
    ...
    def draw_squares(self):
        ...
        self.b_6 = self.canvas.create_rectangle(40, 70, 70, 100, fill="#F0F0F0",
                                                tags=("boxes", "weapon"))  # weapon 70-40 and 100-70
    ...
    def cast_spell(self, event):
        ...
        if self.master.player_obj.equipped_spells[selected_spell] != None:
            if self.master.player_obj.equipped_spells[selected_spell].spellvalue[0] == 'self':
                self.master.charinfo.portait_message_update(
                    self.master.player_obj.cast_spell(self.master.player_obj.equipped_spells[selected_spell].name))
...
class TowerMap(tk.Frame):  # (parent) - Incomplete TODO
    def __init__(self, master, bg='black', fg="white", **args):
        tk.Frame.__init__(self, master, **args)
        self.master = master.master.master.master.master
        # self.master=master
        self.bg = bg
    ...
    def init_map(self):
        self.curr_index = int(list(self.master.floors_arr.keys())[0].split('floor')[1])
        self.element_arr.append(self.canvas.create_rectangle(30, 30, 60, 60, outline=self.fg, fill=self.bg,
                                                             tags=('floor', f'{self.curr_index}')))
        # print(self.curr_index)

    def increment(self):
        self.master.floors_arr[f'floor{self.curr_index + 1}'] = objects.generate_next_level(self.curr_index,
                                                                                            self.master.player_obj)
        self.curr_index += 1
        self.new_floor(self.curr_index, len(self.master.floors_arr) - 1)

    def new_floor(self, floor_num, index):
        x_offset = index % 6 * 60
        if index % 6 == 0:
            self.y_offset = ((self.y_offset / 60) + 1) * 60
        self.element_arr.append(
            self.canvas.create_rectangle(30 + x_offset, 30 + self.y_offset, 60 + x_offset, 60 + self.y_offset,
                                         fill=self.bg, outline=self.fg, tags=("floor", f"{floor_num}")))
        if index % 6 < 6 and index % 6 != 0:
            self.canvas.create_line(0 + x_offset, 45 + self.y_offset, 30 + x_offset, 45 + self.y_offset, fill=self.fg)
        if index % 6 == 0:
            self.canvas.create_line((345, 60 + self.y_offset - 60), (345, 75 + self.y_offset - 60),
                                    (45, 75 + self.y_offset - 60), (45, 90 + self.y_offset - 60), fill=self.fg)
        self.map_hover_text()

    def map_hover_text(self):  # Modify this to format string
        tooltip = ToolTip(self)

        def enter(event):
            hovered_id = event.widget.find_withtag('current')[0]  # self.canvas.gettags(event))
            selected_tag = self.canvas.gettags(hovered_id)[1]
            ##get floor name by floor number
            item_formatted = f"Floor: {selected_tag}\n{self.master.floors_arr['floor' + selected_tag].floor_title}"
            tooltip.show_tooltip(item_formatted)

        def leave(event):
            tooltip.hide_tooltip()

        self.canvas.tag_bind("floor", "<Enter>", enter)
        self.canvas.tag_bind("floor", "<Leave>", leave)
...
class ToolTip():
    ...
    def show_tooltip(self, text):
        ...
        label = tk.Label(tw, text=self.text, justify=tk.LEFT,
                         background="#ffffe0", relief=tk.SOLID, borderwidth=2, width=len(text),
    ...
    def inventory_updated(self):
        ...
        for item in self.master.player_obj.inventory:
            if isinstance(item, objects.Potion):
                ...
                if item.slot == 'weapon':
                    self.inventory.insert('', 'end', text=item.name,
                                          values=(f'{item.bonus[0]} | {item.bonus[1]}', "", ""))
                else:
    ...
    def spells_updated(self):
        ...
        for spell in self.master.player_obj.learned_spells:
            if isinstance(spell, objects.Spell):
                self.learnedspells.insert('', 'end', text=spell.name, values=(
                spell.spellslot, spell.spellvalue[0], spell.spellvalue[1], spell.cost))
...
class Visual_Board(tk.Frame):  # Incomplete 500x700
    ...
    def init_floor(self):
        ...
        if board_width % 2 == 0:
            ...
            top_left_corner = (
            center[0] - ((BOX * (board_width // 2)) + 10), center[1] - ((BOX * (board_width // 2)) + 10))

        for row in range(board_height):
            for column in range(board_width):
                x1, y1 = top_left_corner[0] + (BOX * column), top_left_corner[1] + (BOX * row)
                # x2,y2 = x1+BOX,y1+BOX
                self.canvas_items[(row, column)] = self.canvas.create_text(x1 + 10, y1 + 10, fill=self.board_color,
                                                                           font=('consolas', 16), text=CHAR_DICT[
                        self.master.current_floor.floor[row][column]])
    ...
    def create_shop_window(self, s):
        self.shopframe = self.canvas.create_window((self.canvas.winfo_reqwidth() - 4) // 2,
                                                   (self.canvas.winfo_reqheight() - 4) // 2,
                                                   window=ShopUi(self.master, s))
        self.master.charinfo.toggle_top_state()

    def create_combat_window(self, ev, event):
        self.combatframe = self.canvas.create_window((self.master.winfo_reqwidth()), 0, anchor=tk.NE,
                                                     window=Combat_Window(self.master, ev, event))
        self.master.charinfo.toggle_top_state()
    ...
    def update_current_map(self, prev):
        if prev in self.master.current_floor.events:
            ...
            self.canvas.itemconfig(self.canvas_items[prev],
                                   text=CHAR_DICT[self.master.current_floor.floor[prev[0]][prev[1]]])
        self.canvas.itemconfig(
            self.canvas_items[(self.master.current_floor.player_pos[0], self.master.current_floor.player_pos[1])],
            text=CHAR_DICT['*'])
